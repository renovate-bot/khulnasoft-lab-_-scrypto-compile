...
from scryto_compile.source_unit import SourceUnit
...
class CompilationUnit:
    ...
    def __init__(self, scryto_compile: "ScrytoCompile", unique_id: str):
        """Init the object
        ...
        self._scryto_compile: "ScrytoCompile" = scryto_compile

        if unique_id == ".":
            unique_id = str(uuid.uuid4())

        scryto_compile.compilation_units[unique_id] = self  # type: ignore
    ...
    @property
    def scryto_compile(self) -> "ScrytoCompile":
        """Return the ScrytoCompile object

        Returns:
            ScrytoCompile: Associated ScrytoCompile object
        """
        return self._scryto_compile
    ...
    def create_source_unit(self, filename: Filename) -> SourceUnit:
        ...
        Add the relevant info in the compilation unit/scryto compile
    ...
    def find_absolute_filename_from_used_filename(self, used_filename: str) -> str:
        ...
        if used_filename in self._scryto_compile.filenames:
    ...
    def filename_lookup(self, filename: str) -> Filename:
        ...
        from scryto_compile.platform.truffle import Truffle

        if isinstance(self.scryto_compile.platform, Truffle) and filename.startswith("project:/"):
