...
from scryto_compile.scryto_compile import compile_all, get_platforms
from scryto_compile.scrytoparser import DEFAULTS_FLAG_IN_CONFIG, scrytoparser
from scryto_compile.platform import InvalidCompilation
from scryto_compile.platform.all_export import PLATFORMS_EXPORT
from scryto_compile.utils.zip import ZIP_TYPES_ACCEPTED, save_to_zip
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="""scryto-compile. For usage information,
see https://github.com/khulnasoft-lab/scryto-compile/wiki""",
        usage="scryto-compile contract.sol [flag]",
    )
    ...
    parser.add_argument(
        "--config-file",
        help="Provide a config file (default: scryto_compile.config.json)",
        action="store",
        dest="config_file",
        default="scryto_compile.config.json",
    )

    parser.add_argument(
        "--export-format",
        help=f"""Export json with non scryto-compile format
    ...
    parser.add_argument(
        "--export-dir",
        help="Export directory (default: scryto-export)",
        action="store",
        dest="export_dir",
    ...
    parser.add_argument(
        ...
        version=require("scryto-compile")[0].version,
        action="version",
    )
...
def _print_filenames(compilation: "ScrytoCompile") -> None:
    """Print the filenames

    Args:
        compilation (ScrytoCompile): ScrytoCompile project
    """
...
def _print_libraries(compilation: "ScrytoCompile") -> None:
    for compilation_id, compilation_unit in compilation.compilation_units.items():
        print(
            f"Compilation unit: {compilation_id} solc {compilation_unit.compiler_version.version})"
        )
