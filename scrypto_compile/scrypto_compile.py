"""
ScrytoCompile main module. Handle the compilation.
"""
import base64
import glob
import inspect
...
from scryto_compile.platform import all_platforms
from scryto_compile.platform.solc_standard_json import SolcStandardJson
from scryto_compile.platform.vyper import VyperStandardJson
from scryto_compile.platform.abstract_platform import AbstractPlatform
from scryto_compile.platform.all_export import PLATFORMS_EXPORT
from scryto_compile.platform.solc import Solc
from scryto_compile.platform.standard import export_to_standard
from scryto_compile.utils.naming import Filename
from scryto_compile.utils.npm import get_package_name
from scryto_compile.utils.zip import load_from_zip

# Cycle dependency
if TYPE_CHECKING:
    pass

LOGGER = logging.getLogger("ScrytoCompile")
logging.basicConfig()
...
class ScrytoCompile:
    ...
    def __init__(self, target: Union[str, AbstractPlatform], **kwargs: str) -> None:
        """See https://github.com/khulnasoft-lab/scryto-compile/wiki/Configuration
    ...
    def filename_lookup(self, filename: str) -> Filename:
        """Return a scryto_compile.naming.Filename from a any filename
    ...
    # TODO: refactor import_archive_compilations to rely on one ScrytoCompile object
    # But multiple compilation units
    @staticmethod
    def import_archive_compilations(compiled_archive: Union[str, Dict]) -> List["ScrytoCompile"]:
        ...
        Returns:
            [ScrytoCompile]: List of scryto compile object
        """
        ...
        return [ScrytoCompile(archive) for archive in compiled_archive["compilations"]]
    ...
    def export(self, **kwargs: str) -> List[str]:
        """Export to json.
        The json format can be scryto-compile, solc or truffle.
...
# TODO: refactor me to be integrated within ScrytoCompile.__init__
def compile_all(target: str, **kwargs: str) -> List[ScrytoCompile]:
    """Given a direct or glob pattern target, compiles all underlying sources and returns
    all the relevant instances of ScrytoCompile.
    ...
    Returns:
        List[ScrytoCompile]: Returns a list of ScrytoCompile instances for all compilations which occurred.
    """
    use_solc_standard_json = kwargs.get("solc_standard_json", False)

    # Check if the target refers to a valid target already.
    compilations: List[ScrytoCompile] = []
    if os.path.isfile(target) or is_supported(target):
        if target.endswith(".zip"):
            ...
            compilations.append(ScrytoCompile(target, **kwargs))
