...
from scryto_compile.utils.naming import convert_filename, extract_filename, extract_name
...
class Embark(AbstractPlatform):
    ...
    def compile(self, scryto_compile: "ScrytoCompile", **kwargs: str) -> None:
        ...
        if embark_overwrite_config:
            ...
            if (not "plugins" in embark_json) or (not plugin_name in embark_json["plugins"]):
                raise InvalidCompilation(
                    ...
                    "https://github.com/khulnasoft-lab/scryto-compile/wiki/Usage#embark)"
        ...
        infile = os.path.join(self._target, "scryto-export", "contracts-embark.json")
        ...
        with open(infile, "r", encoding="utf8") as file_desc:
            ...
            if "sources" in targets_loaded:
                compilation_unit.filenames = [
                    convert_filename(
                        path, _relative_to_short, scryto_compile, working_dir=self._target
                    )
            ...
            for k, ast in targets_loaded["asts"].items():
                filename = convert_filename(
                    k, _relative_to_short, scryto_compile, working_dir=self._target
                )
