...
from scryto_compile.platform import solc_standard_json
...
def _handle_bytecode(scryto_compile: "ScrytoCompile", target: str, result_b: bytes) -> None:
    """Parse the bytecode and populate ScrytoCompile info

    Args:
        scryto_compile (ScrytoCompile): Associate ScrytoCompile object
    ...
    compilation_unit = CompilationUnit(scryto_compile, str(target))
    ...
    scryto_compile.bytecode_only = True
...
class Etherscan(AbstractPlatform):
    ...
    def compile(self, scryto_compile: "ScrytoCompile", **kwargs: str) -> None:
        ...
        if source_code == "" and not only_source:
            ...
            _handle_bytecode(scryto_compile, target, html)
            return
        ...
        compilation_unit = CompilationUnit(scryto_compile, contract_name)
