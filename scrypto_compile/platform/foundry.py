...
from scryto_compile.platform.hardhat import hardhat_like_parsing
...
class Foundry(AbstractPlatform):
    ...
    def compile(self, scryto_compile: "ScrytoCompile", **kwargs: str) -> None:
        ...
        if not ignore_compile:
            ...
            if not compile_all:
                foundry_config = self.config(str(scryto_compile.working_dir.absolute()))
                if foundry_config:
                    compilation_command += [
                        "--skip",
        ...
        hardhat_like_parsing(scryto_compile, self._target, build_directory, self._target)
