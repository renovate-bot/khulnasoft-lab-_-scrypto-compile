...
from scryto_compile.utils.naming import convert_filename, extract_name
...
class Buidler(AbstractPlatform):
    ...
    def compile(self, scryto_compile: "ScrytoCompile", **kwargs: str) -> None:
        ...
        Args:
            scryto_compile (ScrytoCompile): Associated ScrytoCompile objects
        ...
        # See https://github.com/khulnasoft-lab/scryto-compile/issues/116
        ...
        if not os.path.isfile(os.path.join(self._target, target_solc_file)):
            if os.path.isfile(os.path.join(self._target, target_vyper_file)):
                ...
                txt += " Please open an issue in https://github.com/khulnasoft-lab/scryto-compile"
                raise InvalidCompilation(txt)
            txt = f"`buidler compile` failed. Can you run it?\n{os.path.join(self._target, target_solc_file)} not found"
            raise InvalidCompilation(txt)

        compilation_unit = CompilationUnit(scryto_compile, str(target_solc_file))
            ...
            if "sources" in targets_json:
                for path, info in targets_json["sources"].items():
                    ...
                    if skip_filename:
                        ...
                        path = convert_filename(
                            path, relative_to_short, scryto_compile, working_dir=buidler_working_dir
                        )
