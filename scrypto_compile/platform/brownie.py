...
from scryto_compile.utils.naming import Filename, convert_filename
...
class Brownie(AbstractPlatform):
    ...
    def compile(self, scryto_compile: "ScrytoCompile", **kwargs: str) -> None:
        ...
        _iterate_over_files(scryto_compile, Path(self._target), filenames)
...
def _iterate_over_files(
    scryto_compile: "ScrytoCompile", target: Path, filenames: List[Path]
) -> None:
    """Iterates over the files and populates the information into the ScrytoCompile object
    ...
    for original_filename in filenames:
        with open(original_filename, encoding="utf8") as f_file:
            ...
            filename: Filename = convert_filename(
                filename_txt, _relative_to_short, scryto_compile, working_dir=target
            )
