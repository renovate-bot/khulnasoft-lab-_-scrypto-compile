...
from scryto_compile.platform.exceptions import InvalidCompilation
from scryto_compile.platform.types import Type
from scryto_compile.utils.naming import (
...
def _build_contract_data(compilation_unit: "CompilationUnit") -> Dict:
    ...
    libraries_to_update = compilation_unit.scryto_compile.libraries
...
def export_to_solc(scryto_compile: "ScrytoCompile", **kwargs: str) -> List[str]:
    ...
    if len(scryto_compile.compilation_units) == 1:
        compilation_unit = list(scryto_compile.compilation_units.values())[0]
...
class Solc(AbstractPlatform):
    ...
    def compile(self, scryto_compile: "ScrytoCompile", **kwargs: str) -> None:
        """Run the compilation
        ...
        compilation_unit = CompilationUnit(scryto_compile, str(self._target))
        ...
        if "sources" in targets_json:
            for path, info in targets_json["sources"].items():
                if skip_filename:
                    ...
                    path = convert_filename(
                        path, relative_to_short, scryto_compile, working_dir=solc_working_dir
                    )
        ...
        for original_contract_name, info in targets_json["contracts"].items():
            ...
            if skip_filename:
                filename = convert_filename(
                    target,
                    relative_to_short,
                    compilation_unit.scryto_compile,
                    working_dir=solc_working_dir,
                )
            else:
                filename = convert_filename(
...
def _run_solc(
    ...
    """Run solc.
    Ensure that scryto_compile.compiler_version is set prior calling _run_solc
