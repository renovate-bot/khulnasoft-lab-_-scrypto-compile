"""
Standard scryto-compile export
"""
import json
import os
...
from scryto_compile.compilation_unit import CompilationUnit
from scryto_compile.compiler.compiler import CompilerVersion
from scryto_compile.platform import Type as PlatformType
from scryto_compile.platform.abstract_platform import AbstractPlatform
from scryto_compile.utils.naming import Filename

# Cycle dependency
from scryto_compile.utils.natspec import Natspec

if TYPE_CHECKING:
    from scryto_compile import ScrytoCompile


def export_to_standard(scryto_compile: "ScrytoCompile", **kwargs: str) -> List[str]:
    """Export the project to the standard scryto compile format
    ...
    output = generate_standard_export(scryto_compile)

    export_dir = kwargs.get("export_dir", "scryto-export")
    if not os.path.exists(export_dir):
        os.makedirs(export_dir)

    target = (
        "contracts"
        if os.path.isdir(scryto_compile.target)
        else Path(scryto_compile.target).parts[-1]
    )
...
class Standard(AbstractPlatform):
    """
    Standard platform (scryto-compile specific)
    """
    ...
    def compile(self, scryto_compile: "ScrytoCompile", **_kwargs: str) -> None:
        """Compile the file (load the file for the Standard platform) and populates the ScrytoCompile object

        Args:
            scryto_compile (ScrytoCompile): Associated ScrytoCompile
        ...
        (underlying_type, unit_tests) = load_from_compile(scryto_compile, loaded_json)
    ...
    def is_supported(target: str, **kwargs: str) -> bool:
        """Check if the target has the standard scryto-compile format
        ...
        Returns:
            bool: True if the target is a scryto-compile generated project
        """
    ...
    def is_dependency(self, path: str) -> bool:
        ...
        This function always return false, the deps are handled by scryto_compile_dependencies
        ...
        Returns:
            bool: Always False
        """
        # handled by scryto_compile_dependencies
        return False
...
def generate_standard_export(scryto_compile: "ScrytoCompile") -> Dict:
    """Convert the ScrytoCompile object to a json

    Args:
        scryto_compile (ScrytoCompile): ScrytoCompile object to export

    Returns:
        Dict: ScrytoCompile converted to a json
    """

    compilation_units = {}
    libraries_to_update = scryto_compile.libraries
    for key, compilation_unit in scryto_compile.compilation_units.items():
        ...
        for filename, source_unit in compilation_unit.source_units.items():
            ...
            for contract_name in source_unit.contracts_names:
                ...
                source_unit_dict[filename.relative]["contracts"][contract_name] = {
                    ...
                    "is_dependency": scryto_compile.is_dependency(filename.absolute),
    ...
    output = {
        ...
        "package": scryto_compile.package,
        "working_dir": str(scryto_compile.working_dir),
        "type": int(scryto_compile.platform.platform_type_used),
        "unit_tests": scryto_compile.platform.guessed_tests(),
        "scryto_version": "0.0.2",
    }
    return output


def _load_from_compile_legacy1(scryto_compile: "ScrytoCompile", loaded_json: Dict) -> None:
    """Load from old (old) export

    Args:
        scryto_compile (ScrytoCompile): ScrytoCompile object to populate
        loaded_json (Dict): Json representation of the ScrytoCompile object
    """
    compilation_unit = CompilationUnit(scryto_compile, "legacy")
    ...
    for path, ast in loaded_json["asts"].items():
        ...
        filename = scryto_compile.filename_lookup(path)
    ...
    for contract_name, contract in loaded_json["contracts"].items():
        ...
        if contract["is_dependency"]:
            compilation_unit.scryto_compile.dependencies.add(filename.absolute)
            compilation_unit.scryto_compile.dependencies.add(filename.relative)
            compilation_unit.scryto_compile.dependencies.add(filename.short)
            compilation_unit.scryto_compile.dependencies.add(filename.used)


def _load_from_compile_legacy2(scryto_compile: "ScrytoCompile", loaded_json: Dict) -> None:
    """Load from old (old) export
    ...
    for key, compilation_unit_json in loaded_json["compilation_units"].items():
        compilation_unit = CompilationUnit(scryto_compile, key)
        compilation_unit.compiler_version = CompilerVersion(
        ...
        for contract_name, contract in compilation_unit_json["contracts"].items():
            ...
            if contract["is_dependency"]:
                scryto_compile.dependencies.add(filename.absolute)
                scryto_compile.dependencies.add(filename.relative)
                scryto_compile.dependencies.add(filename.short)
                scryto_compile.dependencies.add(filename.used)


def _load_from_compile_0_0_1(scryto_compile: "ScrytoCompile", loaded_json: Dict) -> None:
    for key, compilation_unit_json in loaded_json["compilation_units"].items():
        compilation_unit = CompilationUnit(scryto_compile, key)
        compilation_unit.compiler_version = CompilerVersion(
            compiler=compilation_unit_json["compiler"]["compiler"],
...
def _load_from_compile_current(scryto_compile: "ScrytoCompile", loaded_json: Dict) -> None:
    for key, compilation_unit_json in loaded_json["compilation_units"].items():
        compilation_unit = CompilationUnit(scryto_compile, key)
        compilation_unit.compiler_version = CompilerVersion(
            compiler=compilation_unit_json["compiler"]["compiler"],
...
def load_from_compile(scryto_compile: "ScrytoCompile", loaded_json: Dict) -> Tuple[int, List[str]]:
    """Load from a standard scryto compile json
    ...
    """
    scryto_compile.package_name = loaded_json.get("package", None)
    if "compilation_units" not in loaded_json:
        _load_from_compile_legacy1(scryto_compile, loaded_json)

    elif "scryto_version" not in loaded_json:
        _load_from_compile_legacy2(scryto_compile, loaded_json)

    elif loaded_json["scryto_version"] == "0.0.1":
        _load_from_compile_0_0_1(scryto_compile, loaded_json)
    else:
        _load_from_compile_current(scryto_compile, loaded_json)

    scryto_compile.working_dir = loaded_json["working_dir"]
