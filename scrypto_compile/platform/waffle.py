...
from scryto_compile.platform.abstract_platform import AbstractPlatform, PlatformConfig
...
class Waffle(AbstractPlatform):
    ...
    def compile(self, scryto_compile: "ScrytoCompile", **kwargs: str) -> None:
        """Compile the project and populate the ScrytoCompile object
        ...
        if "sources" in target_all:
            compilation_unit.filenames = [
                convert_filename(path, _relative_to_short, scryto_compile, working_dir=target)
        ...
        for contract in target_all["contracts"]:
            ...
            filename = convert_filename(
                contract[0], _relative_to_short, scryto_compile, working_dir=target
            )
